...
#define MOTOR1 7                              // определим номер вывода Arduino, к которому подключается мотор
#define MOTOR2 8                              // определим номер вывода Arduino, к которому подключается мотор
#define MOTOR3 9                              // определим номер вывода Arduino, к которому подключается мотор
...
void setup() {
  ...
  pinMode(MOTOR1, OUTPUT);                    // конфигурируем вывод MOTOR на Arduino как выход, см. описание  pinMode() http://arduino.ru/Reference/PinMode
  digitalWrite(MOTOR1, LOW);                  // выключаем мотор, см. описание digitalWrite(): http://arduino.ru/Reference/DigitalWrite
  pinMode(MOTOR2, OUTPUT);                    // конфигурируем вывод MOTOR на Arduino как выход, см. описание  pinMode() http://arduino.ru/Reference/PinMode
  digitalWrite(MOTOR2, LOW);                  // выключаем мотор, см. описание digitalWrite(): http://arduino.ru/Reference/DigitalWrite  
  pinMode(MOTOR3, OUTPUT);                    // конфигурируем вывод MOTOR на Arduino как выход, см. описание  pinMode() http://arduino.ru/Reference/PinMode
  digitalWrite(MOTOR3, LOW);                  // выключаем мотор, см. описание digitalWrite(): http://arduino.ru/Reference/DigitalWrite 
...
void sendData() {
 ...
 if (sData > threshold) {                      // если размах сигнала больше порога срабатывания
    digitalWrite(MOTOR1, HIGH);                // устанавливаем логическую "1" на выход MOTOR (высокий уровень напряжения на выходе - мотор включается)
    digitalWrite(MOTOR2, HIGH);                // устанавливаем логическую "1" на выход MOTOR (высокий уровень напряжения на выходе - мотор включается)
    digitalWrite(MOTOR3, HIGH); }               // устанавливаем логическую "1" на выход MOTOR (высокий уровень напряжения на выходе - мотор включается)
 else
 {   digitalWrite(MOTOR1, LOW);                 // иначе устанавливаем логический "0" на выход MOTOR (низкий уровень напряжения на выходе - мотор выключается)
    digitalWrite(MOTOR2, LOW);                 // иначе устанавливаем логический "0" на выход MOTOR (низкий уровень напряжения на выходе - мотор выключается)
    digitalWrite(MOTOR3, LOW);                 // иначе устанавливаем логический "0" на выход MOTOR (низкий уровень напряжения на выходе - мотор выключается)
} }
